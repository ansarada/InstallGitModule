...
# Synopsis: Imports the module into the Local User path so it can then be published.
task Archive BeforeArchive, {
	...
	$dest = 'C:\Users\vagrant\Documents\WindowsPowerShell\Modules'
	$psd1Modules = @(Get-ChildItem -Path $ModulePath -Include *.psd1 -Recurse)
